...
10 + 100
```
or by using a ***function*** like `exp`, which take as input a number returns the exponential of that number
```{r}
exp(10)
```
...
print("hello world")
```
To see this, we can use the `str` function, which tells us about the "structure" of any object. In this case, `str` just tells us that `print` is a function:
```{r}
str(print)
```
...
- Use "tabs" to set off blocks of code like loops and function definition. Good coding style will make these blocks look obvious since they are indented and spaced nicely. This makes understanding which parts of the code are run when much easier. In fact, some languages (*cough* Python *cough*) require the use of indentation to define code blocks.
...
- Err on the side of using longer object names that describe what that object stores: e.g., `populationSize` is often better than `N`. Note that R itself often doesn't obey this rule; e.g., the function `c` combines values. **Never** name a function a single letter.
...
In our examples of how R creates vectors, a more general issue arises about how R responds when you do something maybe not quite right. When we created the `mixed` vector above, R silently converted the numbers to strings *assuming that's what you wanted to do*. Of course, maybe you didn't want that in which case your vector doesn't contain the values you expect. Similarly, if we put a function in a vector with a string,
```{r}
c(print, "hello world")
```
R assumes here we wanted to create a list and returns one. Maybe this is more understandable, since a list is the only way to group a function and a string together in a simple way. What these two scenarios share in common is that R could have thrown an error message when it received something unusual but instead it silently returned something that assumes the user intent; when it assumes correctly, this may work out for the user, but not so when it assumes incorrectly. In addition, the user may develop incorrect assumptions about how R works based on these silent assumptions. One takeaway from this is that the **lack of an error message doesn't necessarily mean that your R code is doing what you expect**. Another is that understanding the basics of how R code works is always helpful for troubleshooting issues as you can fall back on that knowledge when your assumptions about what should work based on past experience fail you.
...
Functions are important so we will discuss them in more detail. Functions take ***arguments*** between the parentheses and these arguments are the data you want the function to use. For example, the `runif` function generates `r`andom numbers with a `unif`orm distribution. There are three arguments to the function:
```{r}
str(runif)
```
...
```
Notice that giving the name of the argument allows us to give the arguments in any order. If an arguments doesn't have a name, then you must give the arguments in exactly the order they are listed in the definition of the function (see the help for a function with `?` to get info on the arguments the function takes).
...
1.  Write a function that takes a single vector argument and prints out the values from the vector that are greater than zero. For example, if you create a vector of normally distributed random numbers, 
    ```{r}
    #| eval: false
    rvec = rnorm(100)
    ```
    ...
    your_fuction(rvec)
    ```
    would then output something like `[1] 0.261483381 0.174284945 1.411168631 0.744854041 1.940994378 ...`.
    
    Tip. Use a `for` loop. Or you can use another way if you know it.
...
3.  Suppose you are working on same data with a `messy_months` column specifying the month the data were collected. However, the person generating the data did not use a consistent way of inputting the month and alternated between either a number, a three letter abbreviation, or the full month name. 

    Write a function that takes as input the `messy_months` vector below, which represents this messy month column, and returns an output vector where the months are now all either a number, three letter abbreviation, or full name (your choice as to which to output). In short, your function should take the `messy_months` vector and return a cleaned up version with only a single format for representing the month.

    In your submitted `qmd` file, include the code below so that you have the `messy_months` vector loaded.
    ```{r}
    #| warning: false
    library(tidyverse)
    messy_months = read_csv("months.csv") |> pull(month)
    ```
