...
```
The data are collected for each week and the `week_end_date` column gives the last day of the week for that row of data. The `state` column gives the US state or territory for that row of data. The other columns give various hospitalization and death metrics for COVID-19 and other respiratory infections in that state during that week. See the links above for more information on what each column means.
...
In the `qmd` that you submit, **please include the code chunk above that loads in the data and creates the `us_hosps_deaths` data table as the first code chunk in the markdown file that you submit for this assignment.** That way, the table will be present when I run the code in your markdown file.
...
This is a big table so use the `str(us_hosps_deaths)` function to get information on all the columns and `view(us_hosps_deaths)` will bring up the GUI viewer of the table.

1.  Which **state** had the most **COVID-19 deaths** during one week in 2021? Which week was that?\
...
2.  How many COVID-19 deaths did Kentucky have during 2021?\
