...
```{r}
fac_factor = factor(c("Famulski", "Burger", "Seifert", "Santollo", "Duncan", "Long"))
fac_factor
```
...
Note that each of these commands returned a **new** `data.frame` and the original is unchanged until we explicitly save back to that variable name:
```{r}
dframe
```
...
2.  What date had the highest number of new deaths in the United States?\
...
6.  In 2021, on how many days did the United States have fewer than 0.4 deaths per million people due to COVID-19? What is answer the United Kingdom? Use the column `new_deaths_per_million` to answer this question.
