...
# One format to rule them all: tidy data ![](assets/one-ring.png){width=50px}
...
2.  The `RISK ALLELE FREQUENCY` column in the GWAS data contains the frequency of the allele the increases the risk of the `DISEASE/TRAIT` in the population. For the three `DISEASE/TRAIT`s "Prostate cancer", "Lung cancer", and "Breast cancer", calculate the average value of the `RISK ALLELE FREQUENCY` for each disease. Which disease has the highest average `RISK ALLELE FREQUENCY`?
