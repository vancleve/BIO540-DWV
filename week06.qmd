...
us_hosps  = soc_read("https://data.cdc.gov/api/odata/v4/aemt-mg7g") |> as_tibble()
us_deaths = soc_read("https://data.cdc.gov/api/odata/v4/r8kw-7aab") |> as_tibble()
  ...
  inner_join( # join the two tables together
    us_hosps |>
      ...
      inner_join(tibble(state_abbrv = state.abb, state = state.name) |> 
                  add_row(state_abbrv = c("USA", "DC", "PR"), state = c("United States", "District of Columbia", "Puerto Rico"))), by = join_by(week_end_date, state))
```
Looking at the `soc_read` commands, you'll notice that we start with two separate files, one for hospitalization and one for deaths. It turns out that the CDC gives us these tables **separately**: 
[hospitalizations](https://data.cdc.gov/Public-Health-Surveillance/Weekly-United-States-Hospitalization-Metrics-by-Ju/aemt-mg7g/about_data) and [deaths](https://data.cdc.gov/National-Center-for-Health-Statistics/Provisional-COVID-19-Death-Counts-by-Week-Ending-D/r8kw-7aab/about_data). Putting these data tables together requires "joining" them. Not only that, we have to do some extra work to even make the join work, which we accomplish with *another join*. By learning about joins, we'll not only be able to understand this example, we'll be able to combine data from all kinds of places.
...
us_deaths |> 
  ...
  filter(n > 1)
```
Ruh roh. It looks like `NA` and state coming up 82 times! Let's check out why by filtering those `NA` dates
```{r}
...
states
```
How can we combine the state names from `states` into `us_hosps`? We can this with a join! To see this, let's join the new states table with our `us_hosps`.
```{r}
us_hosps_w_states = 
  us_hosps |>
...
us_hosps_w_states
```
We can see that R told us when doing this join that it's joining `` with `by = join_by(state_abbrv)` ``. This actually means R tried to guess which key variables it should use to join the tables. It did this by look at columns in both tables and finding ones with identical names. This is called a **natural join**. In this specific case, the only column with the same name in both tables is `state_abbrv` (which we renamed from `jurisdiction`), so that is the primary and foreign key that was used for the join. Also note that since this is a left join and `us_hosps` is the primary table, all of its rows are kept and we simply get the new `state` column added.

Now we are ready to join the deaths and hospitalization tables together since we have full state names in both tables. We do this by telling `inner_join` which variables are the primary and foreign keys since they have different names in the two tables (and thus not have to rename the columns all the time).
```{r}
us_hosps_deaths =
us_deaths |>
...
```{r}
filter_dt = tibble(state = c("California", "Kentucky", "New York", "Texas"))
```
...
us_hosps_deaths |>
  semi_join(filter_dt)
```
The `anti_join` works analogously; here, we can use it to note something about what happened when we gave `us_hosps` its full state name. If we look at the different `jurisdiction`s in `us_hosps`, we see that there are more than the 50 states plus the "USA", "DC" and "PR", which we put in our `states` table.
```{r}
us_hosps |> distinct(jurisdiction)
```
To figure out what those extra jurisdictions are, we can use `anti_join` to find the rows in `us_hosps` that don't have a match in `states`:
```{r}
us_hosps |>
...
The package `dbplyr` let's us access the database as if we were using our normal `dplyr` commands but in the background SQL commands are sent to the database. It's loaded for us automatically through `library(tidyverse)`. Let's look at the trips table using the `dplyr` function `tbl`.
```{r}
sftrips = dbcon |>
  tbl("trips")
sftrips
```
You can see that the table shows the number of columns but not the number of rows. This is because the table is loaded **lazily**, which means data aren't accessed until they must be. In fact, if we ask how many rows are in this table, `R` just throws up its hands:
```{r}
sftrips |> 
  nrow()
```
Now, suppose we wanted to collect all the bike trips that were taken on July 4th, which is a US holiday, and we wanted to see which station generated the longest trips. We could use our normal `dplyr` tools as follows:
```{r}
july4th = sftrips |> 
...
1.  Find the day that has the worst departure delay.
...
2.  Imagine you’ve found the top 10 most popular destinations using this code:
    ...
    top_dest = flights |>
      ...
      head(10)
    ```
    Wrangle the `flights` table so that you isolate only the flights to those destinations.

3.  Add the latitude and the longitude of the origin and destination airport to the `flights` table.

4.  Find the tail numbers that don’t have a matching record in planes; what two airlines do most of these planes belong to?
