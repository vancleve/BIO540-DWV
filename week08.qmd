...
```{r}
plottitle = "Life expectancy over time in the Gapminder data" # save space in the code
plot(gapminder$year, gapminder$lifeExp, 
...
To understand how `ggplot2` works, first consider how the following example replicates the first life expectancy by year plot above:
```{r}
ggplot(data = gapminder) + 
  geom_point(aes(x = year, y = lifeExp))
```
(Note that we don't need to load the `ggplot2` library as it is already loaded as part of `library(tidyverse)` above.)

The first line above has the function `ggplot`, which creates a coordinate system for the plot that you can add "layers" to. The argument to `ggplot` indicates which dataset to use. 

Adding layers is accomplished by "adding" to the base plot with **`+`** and additional features via additional functions. The first feature added above is `geom_point()`, which adds a layer of points and creates a scatter plot. `ggplot2` comes with many other "geometries", which are really just kinds of visual objects, like lines and boxes.
...
In addition to the `x` and `y` axes, you can map variables to the color of the point. Above, you did this using the `country` variable. Now, to repeat this with `ggplot()`, we can do
```{r}
usukjpcn |> ggplot() + 
...
```
Now, `facet_wrap` plots all the possible combinations of `continent` and `year > 1990`. Just like many other `dplyr` and `tidyverse` functions, `vars` let's us specify more than just a variable; rather, we can use an expression with a variable like `year > 1990`.
...
1.  Use the GWAS cancer data (`gwas-catalog-v1.0-associations-e115-r2026-01-19-cancer.tsv`): 
...
3.  Create a plot like the one in Problem 2
    - Except, the new plot should have multiple subplots where each subplot is a different continent. 
...
4.  We'll use a CDC dataset on COVID-19 mortality for this question:\
    ...
    - Make a point plot of the covid death rate in `crude_rate` vs the date for 2020-2021.\
      ...
      Think carefully about what might explain any differences you see between these groups (e.g., access to healthcare, etc).
    - Make a point plot of the covid death rate in `crude_rate` vs the date for 2020-2021.\
