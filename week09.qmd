...
covid_net_hosps = soc_read("https://data.cdc.gov/api/odata/v4/6jg4-xsqq") |> 
  as_tibble() |>
  filter(!is.na(state), state != "All", state != "COVID-NET") |>
  rename(age = agecat_label, sex = sex_label, race = race_label) |>
  mutate(week_ending_date = ymd(week_ending_date), weekly_rate = parse_number(weekly_rate), cumulative_rate = parse_number(cumulative_rate)) |>
  arrange(week_ending_date) |>
  mutate(month = factor(months(week_ending_date), 
                        levels = c("January", "February", "March", "April", "May", "June", 
                                   "July", "August", "September", "October", "November", "December"))) |>
  relocate(month, .after = week_ending_date)
```
Let's work on a slice of these data with just the cases in Colorado, Georgia, and Tennessee for 2021:
```{r}
co_ga_tn_cases_2021 = 
  covid_net_hosps |>
  filter(state %in% c("CO", "GA", "TN")) |>
  filter(year(week_ending_date) == 2021)
...
co_ga_tn_plot = co_ga_tn_cases_2021 |> 
  ...
  ggplot(aes(x=week_ending_date, y=weekly_rate, color=state))
...
covid_net_hosps_exceed_50_per_100k = 
  covid_net_hosps |>
  filter(age == "All", race != "All", sex == "All") |>
  mutate(exceed_50_per_100k = weekly_rate >= 50) |>
...
covid_net_hosps |> 
  filter(age == "All", race != "All", sex == "All") |>
  ggplot() +
  geom_histogram(aes(x = weekly_rate))
```
...
covid_net_hosps |> 
  filter(age == "All", race != "All", sex == "All") |>
  ggplot() +
  geom_histogram(aes(x = log10(1 + weekly_rate)))
```
...
covid_net_hosps |> 
  filter(age == "All", race != "All", sex == "All") |>
  ggplot() +
  geom_histogram(aes(x = log10(1 + weekly_rate), fill=race))
```
...
covid_net_hosps |> 
  filter(age == "All", race != "All", sex == "All") |>
  ggplot() +
  geom_histogram(aes(x = log10(1 + weekly_rate), fill=race), position = "identity", alpha = 0.5)
```
...
covid_net_hosps |> 
  filter(age == "All", race == "Black, non-Hispanic" | race == "White, non-Hispanic", sex == "All") |>
  ggplot() +
  geom_freqpoly(aes(x = log10(1 + weekly_rate), color=race))
```
...
covid_net_hosps |> 
  filter(age == "All", race == "Black, non-Hispanic" | race == "White, non-Hispanic", sex == "All") |>
  ggplot() +
  geom_freqpoly(aes(x = log10(1 + weekly_rate), color=race), binwidth = 0.2)
```
...
covid_net_hosps |> 
  filter(age == "All", race == "Black, non-Hispanic" | race == "White, non-Hispanic", sex == "All") |>
  ggplot() +
  geom_density(aes(x = log10(1 + weekly_rate), fill = race), alpha = 0.3)
```
...
covid_net_hosps |> 
  filter(age == "All", race == "All", sex == "All") |>
  ggplot() +
  geom_histogram(aes(x = log10(1 + weekly_rate), fill = state), alpha = 0.3)
```
...
covid_net_hosps |> 
  filter(age == "All", race == "All", sex == "All") |>
  ggplot() +
  geom_point(aes(x = state, y = log10(1 + weekly_rate))) + 
  coord_flip()
```
...
covid_net_hosps |> 
  filter(age == "All", race == "All", sex == "All") |>
  ggplot(aes(x = state, y = log10(1 + weekly_rate))) +
...
covid_net_hosps |> 
  filter(age == "All", race == "All", sex == "All") |>
  ggplot(aes(x = reorder(state, weekly_rate, FUN = median), y = log10(1 + weekly_rate))) +
  geom_boxplot(outlier.color = "red") +
  coord_flip()
```
...
covid_net_hosps |> 
  filter(age == "All", race == "All", sex == "All") |>
  ggplot(aes(x = reorder(state, log10(1 + weekly_rate), FUN = median), y = log10(1 + weekly_rate))) +
  geom_violin() +
  coord_flip()
```
...
3. Use the imprinting data from Babak et al. (2015) (`babak-etal-2015_imprinted-mouse_tidy.csv`):
    ...
    - Make the part of the histogram for positive values a different color from the part with negative values.
...
4.  Use the following CDC COVD-19 death data for this problem. It comes from the CDC WONDER database (<https://wonder.cdc.gov/>).
    ```{r}
    cdc_data =
    read_tsv("cdc_covid19_mort_rates_state-age-sex_2020-2023.tsv") |>
    mutate(Age = fct_relevel(Age, "< 1 year", "1-4 years", "5-14 years", "15-24 years", "25-34 years", "35-44 years", "45-54 years", "55-64 years", "65-74 years", "75-84 years", "85+ years"))
    ```
...
5.  Use CDC data from above. 
    ...
    - What is most notable in the 2020-2022 years compared to seasonal flu, which tends to have a **single peak** in intensity in the winter?