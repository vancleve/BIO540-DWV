...
You might notice that the `geom_text`s are located right on top of the points they are meant to label. In other words, the text is centered both horizontally and vertically over the location of the point. This can be modified with by changing the "justification" in reference to the (x,y) location of the point you specify. You can set the vertical (`vjust`) and horizontal (`hjust`) justication above using the options below.

![Combinations of horizontal and vertical justification options](assets/just.png){width=50%}

By using `vjust = 'bottom'` and `hjust = 'center'`, we can put the text centered horizontally and above the points. Moreover, we can add `nudge_y = 10` to lift the text a little bit too (`nudge_x` likewise moves the label in the `x` direction):
```{r}
plot_deaths75to84_ky_ny_fl + geom_point(alpha = 0.3) + 
  geom_smooth(lwd = 0.5, span = 0.1) +
  labs(title = "COVID-19 Deaths ages 75-84", x = "Date", y = "Deaths per 100k persons", color = "State") +
  geom_text(aes(label = c("Florida max", "New York max", "Kentucky max")), data = maxdeaths_ky_ny_fl, show.legend = FALSE, vjust = 'bottom', hjust = 'center', nudge_y = 10)
```

Say that say we wanted a more generic annotation located somewhere in the middle of the plot. We now know that we could do that by creating a small data frame like so
```{r}
#| message: false
...
```
However, `ggplot` does give a litte convenience function called `annotate` so that we do not have to create a new data frame. We simply give `annotate` the `x` and `y` axis values where we want the annotate directly. 
```{r}
plot_deaths75to84_ky_ny_fl + geom_point(alpha = 0.3) + 
  geom_smooth(lwd = 0.5, span = 0.1) +
  labs(title = "COVID-19 Deaths ages 75-84", x = "Date", y = "Deaths per 100k persons", color = "State") +
  annotate(geom = "text", x = ymd("2022-01-01"), y = 250, label = "This is a label in the middle of the plot", vjust = "bottom", hjust = "center", color = "black")
```
...
1.  Create a plot of per capita deaths over time using the `cdc` COVID-19 dataset below
    ...
    c. For each subplot, use color to denote each state in the plot.
    d. Use the `labs` function to add x and y labels.
    e. Add `geom_text` annotation for the days with the highest death counts for each subplot 
    f. Rotate the x-axis tick labels so that they are vertical using the `guides` function (see <https://ggplot2-book.org/scales-position#sec-guide-axis> for more info).
...
2.  Create a plot from any of the datasets we have used previously that includes the following
    ...
    d. Non-ggplot2 default theme (pick one of your choice)
