...
2.  Load the GWAS data using the commands below:
    ```{r}
    #| eval: false
    gwas = read_tsv("gwas-catalog-v1.0-associations-e115-r2026-01-19-cancer.tsv", na=c("NA", "NR"), col_types = cols(CHR_POS = col_number()))
    gwas = gwas %>% mutate(risk_allele_freq = parse_double(str_extract(`RISK ALLELE FREQUENCY`, "0\\.\\d+")))
    ```
    - Filter to keep **four** cancer traits of your choosing in the table.
    - Filter for `PVALUE_MLOG` less than 20.
    ...
    - Use `facet_wrap` to make a plot for each `DISEASE/TRAIT`
    - Use a perceptually uniform color map.
    - Add appropriate plot title and axes labels.
    - Save the figure as ".pdf" and **include the .pdf** in the **.zip** that you submit.
...
3.  Use the bike share ride data from San Francisco for the month of July using the commands below ():
    ...
    - Use the `stat_density_2d` function so that unknown values are imputed with a KDE (kernel density estimate).
    - Use a perceptually uniform color map.
    - Add appropriate plot title and axes labels.
    - Save the figure as ".pdf" and **include the .pdf** in the **.zip** that you submit.
