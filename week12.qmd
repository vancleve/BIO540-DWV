...
2.  For this question, we will use genetic data from a few European populations gathered as part of the CEPH-Human Genome Diversity Panel. The file can be loaded using the code below. The `ID` column is an identifying string for the individual, the `Group` column gives the individual's geographic ancestry, and the remaining columns are SNPs.
    ```{r}
