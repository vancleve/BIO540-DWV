...
library(sand) # package the accompanies the book ‘Statistical Analysis of Network Data with R, 2nd Edition’ from Kolaczyk and Csárdi, 2020
data(Ecoli.data)
...
g1 = make_graph(edges = c(1,2, 2,3, 3,4, 4,2), directed = FALSE)
plot(g1)
...
```{r}
tibble(betweenness = betweenness(mg)) |>
  ggplot() + 
...
```{r}
tibble(eigen_centrality = eigen_centrality(mg)$vector) |>
  ggplot() + 
  geom_histogram(aes(x = eigen_centrality))
```
...
1. Use the data file `rugosa_social_network.csv`, which describes the social network of the sleepy lizard (*Tiliqua rugosa*)[^4] as an **adjacency matrix**. You may plot using the basic `igraph` functions or the `ggraph` ones.    
    - Read in the file and convert it to a matrix to create a graph.
    - There are `NA`s in the data; you'll need to convert those to `0` before creating the matrix for the graph.
    - Use the function `column_to_rownames(YOURMATRIX, var = "ID")` before converting the data frame to a matrix since the graph functions like matrices with rownames.
    - Make sure the function you use to create your graph knows whether the graph is weighted or directed.
